...
public class PutInSequenceTest extends FileWritingTest {

  @Test
  public void testPutInSequence()
    ...
    GenerationTask dummyTask = new GenerationTask("TechnischeLogistikSS21", "Kommisionierung",
        "Schritte_Kommissioniervorgang_Seq");
    dummyTask.initMissingWithDefaults();
    dummyTask.getParameters().put("difficulty", 8);
    FdlNode node = c.getFdlnodes().get("Schritte_Kommissioniervorgang_Seq");

    PutInSequence exercise = new PutInSequence(node, dummyTask);
  ...
  @Test
  public void testPutInSequenceJsonSolution()
      throws IOException, DslException, InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException, ModelException {
    Configuration cfg = new Configuration(Configuration.VERSION_2_3_28);
    try {
      cfg.setDirectoryForTemplateLoading(new File("src/main/resources/templates"));
    ...
    Map<String, Object> map = mapper.readValue(jsonSolution, new TypeReference<>() {
    });
    ...
    assertThat("SOLUTION_FORMAT is defined in map", map.get("SOLUTION_FORMAT"), is("MULTI_STRING"));
