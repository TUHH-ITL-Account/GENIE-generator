...
public class SelectCorrectFormulaFromMCTest extends FileWritingTest {

  @Test
  public void testSelectCorrectFormulaFromMC()
    ...
    GenerationTask dummyTask = new GenerationTask("TechnischeLogistikSS21", "Stückgut",
        "Massestrom_Eq1");
    dummyTask.setRandomInstance(new Random());
    dummyTask.setParameters(parameters);
    FdlNode node = c.getFdlnodes().get("Massestrom_Eq1");
    SelectCorrectFormulaFromMC exercise = new SelectCorrectFormulaFromMC(node, dummyTask);
  ...
  @Test
  public void testSelectCorrectFormulaFromMCJsonSolution()
      throws IOException, DslException, InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException, ModelException, UnfulfillableException {
    Configuration cfg = new Configuration(Configuration.VERSION_2_3_28);
    try {
      cfg.setDirectoryForTemplateLoading(new File("src/main/resources/templates"));
