...
public class InputHierarchyLabelsTest extends FileWritingTest {

  @Test
  public void testInputHierarchyLabels()
    ...
    GenerationTask dummyTask = new GenerationTask("TechnischeLogistikSS21", "Transporttechnik",
        "Automatische_Verladesysteme_Hie");
    dummyTask.initMissingWithDefaults();
    dummyTask.getParameters().put("difficulty", 5);
    FdlNode node = c.getFdlnodes().get("Automatische_Verladesysteme_Hie");

    InputHierarchyLabels exercise = new InputHierarchyLabels(node, dummyTask);
  ...
  @Test
  public void testInputHierarchyLabelsJsonSolution()
      throws IOException, DslException, InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException, ModelException, UnfulfillableException {
    Configuration cfg = new Configuration(Configuration.VERSION_2_3_28);
    try {
      cfg.setDirectoryForTemplateLoading(new File("src/main/resources/templates"));
